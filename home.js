...
function generateBooks(container, count) {
    const categories = ['dictionaries', 'languages', 'academic', 'modern', 'bestsellers'];
    const bookTitles = [
    ...
    for (let i = 0; i < count; i++) {
        ...
        const bookCard = createBookCard(title, author, category, price);
        container.appendChild(bookCard);
    }
}

function createBookCard(title, author, category, price) {
    ...
    card.innerHTML = `
        <div class="book-image">
            <img src="https://via.placeholder.com/300x400/1a1a1a/ffffff?text=${encodeURIComponent(title)}" alt="${title}">
            <div class="book-overlay">
                <span class="book-category">${getCategoryName(category)}</span>
            </div>
        </div>
        <div class="book-content">
            <h3 class="book-title">${title}</h3>
            <p class="book-author">${author}</p>
            <p class="book-description">كتاب متخصص في مجال ${getCategoryName(category)} يقدم محتوى قيماً للمهتمين بهذا المجال.</p>
            <div class="book-footer">
                <span class="book-price">${price} ج.م</span>
